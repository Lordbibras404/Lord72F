...
function initApp() {
  ...
  applyTheme();
  generateSplashStars();
  /* Splash: minimum 3.5s for professional loading animation */
...
function showScreen(id) {
  ...
  document.querySelectorAll('.screen').forEach(s => s.classList.remove('active','entering','leaving'));
  const target = document.getElementById(id);
  if (target) {
    target.classList.add('active','entering');
    // FIX: scroll the .content inside, not the screen itself
